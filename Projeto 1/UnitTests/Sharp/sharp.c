...
uint16_t sharp_read(void) {
	...
	if(PORTEbits.RE8 == 1) {
		// Read the ADC
		//adc_start();
		//uint16_t adc_val = adc_read();		// Does not work with unit tests
		uint16_t adc_val = ADC1BUF0;
...
/*
 * adc.h
 * Could not import the file to perform the unit test so had to copy the content to this file
 */
